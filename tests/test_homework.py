...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        ...
        assert re.search(
            r'<\s*title\s*>\s*Записи\s+сообщества\s+' + group.title + r'\s+\|\s+Wetube\s*<\s*\/title\s*>',
            html
        ), 'Отредактируйте HTML-шаблон, не найдено название страницы `<title>Записи сообщества {{ название_группы }} | Wetube</title>`'
        assert re.search(
